...
class SuperviseAction(BaseDynamicAction):
    ...
    async def run(self, **kwargs):
        pretext = 'In the following section you will be given a chat history, a prompt and an answer to the prompt by ' \
                  f'an AI ChatBot.\nPlease decide as to whether the answer is correct and appropriate or not.\n' \
...
class SuperviseVoteAction(BaseDynamicAction):
    ...
    async def run(self, **kwargs):
        prompt = 'In the following section you will be given a prompt and multiple answers to the prompt by ' \
                 ...
                 f'{"chosen": index of the best answer as int}\n' \
...
class SupervisorRole(BaseRole):
    ...
    def __init__(self):
        super().__init__(actions=[SuperviseAction])
    ...
    async def _act(self, *args, **kwargs) -> Message:
        ...
        rsp = await todo.run(chat_history=chat_history, prompt=user_input, answer=answer)
        msg = Message(content=rsp, role=self.profile, cause_by=todo)
...
class Team:
    ...
    async def __supervisors_run(self, prompt, answer, chat_history) -> list[dict]:
        ...
        :return: supervisor feedback
        """
        feedback: list[dict] = list()
        for supervisor in self.supervisors:
            feedback.append((await supervisor.run(prompt=prompt, answer=answer, chat_history=chat_history)).content)
    ...
    async def __call__(self, prompt) -> str:
        ...
        for response, agent in zip(responses, self.agents):
        ...
        while not (correct_response := await Team.__check_response_feedbacks(responses, response_feedbacks)):
        ...
        if len(correct_response) > 1:
            return await self.__supervisor_vote(prompt, correct_response)

        return correct_response[0]
